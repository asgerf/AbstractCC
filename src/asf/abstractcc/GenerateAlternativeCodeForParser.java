...
public class GenerateAlternativeCodeForParser extends DepthFirstAdapter {
    ...
    GenerateAlternativeCodeForParser(File pkgDir, String aParsedAltName,
            String raParsedAltName, BufferedWriter file,
            ResolveTransformIds transformIds, ComputeCGNomenclature CG,
            ComputeSimpleTermPosition CTP, Map simpleTermTransformMap,
            MacroExpander macros, LinkedList listSimpleTermTransform,
    ...
    public void inAAltTransform(AAltTransform node) {
        ...
        for (int i = 0; i < temp.length; i++) {
            ...
            try {
                printVariableDeclaration(type_name, position);
            } catch (IOException e) {
        ...
        try {
            for (int i = 0; i < temp.length; i++) {
                if (simpleTermTransformMap.get(temp[i]) != null) {
                    ...
                    type_name = (String) CG.getAltTransformElemTypes().get(
                            temp[i]);
                }

                position = ((Integer) CG.getTermNumbers().get(temp[i]))
                        .intValue();
                ...
                macros.apply(file, "ParserNewBodyListAdd", new String[] {
                        type_name, "" + position });
    ...
    private void printVariableDeclaration(String type_name, int position) throws IOException {
        if (type_name.equals("Lnull")) {
            macros.apply(file, "ParserListVariableDeclaration", 
                    new String[] {"" + position, "Node<?>" });
        } else if (type_name.startsWith("L")) {
            macros.apply(file, "ParserListVariableDeclaration", 
                    new String[] {"" + position, type_name.substring(1) });
        } else if (type_name.equals("null")) {
            macros.apply(file, "ParserNullVariableDeclaration",
                    new String[] { "" + position });
        } else {
            macros.apply(file, "ParserSimpleVariableDeclaration", 
                    new String[] {type_name, type_name.toLowerCase(), "" + position });
        }
    }
    ...
    public void inASimpleTerm(ASimpleTerm node) {
        try {
            ...
            int elemPosition = ((Integer) CTP.elems_position.get(termKey))
                    .intValue();
            int positionMap = 0;

            if (node.getSimpleTermTail() != null) {
                if (!listSimpleTermTransform.contains(node.getId().getText())) {
                    String type = (String) CTP.positionsMap.get(realcurrentAlt
                            + "." + node.getId().getText());
                    LinkedList list = (LinkedList) transformIds
                            .getProdTransformIds().prod_transforms.get(type);
                    if (list.indexOf(node.getSimpleTermTail().getText()) >= 0) {
                        positionMap = list.indexOf(node.getSimpleTermTail()
                                .getText());
                    }
                }

                if (simpleTermOrsimpleListTermTypes.get(node) != null) {
                    String type = (String) simpleTermOrsimpleListTermTypes
                            .get(node);
                    LinkedList list = (LinkedList) transformIds
                            .getProdTransformIds().prod_transforms.get(type);
                    if (list.indexOf(node.getSimpleTermTail().getText()) >= 0) {
            ...
            macros.apply(file, "ParserSimpleTerm",
                    new String[] { type_name.toLowerCase(), "" + position,
    ...
    public void inAListTerm(AListTerm node) {
        try {
            ...
            for (int i = 0; i < temp.length; i++) {
                ...
                int position = ((Integer) CG.getTermNumbers().get(temp[i]))
                        .intValue();
    ...
    public void outAListTerm(AListTerm node) {
        try {
            Object temp[] = node.getListTerms().toArray();
            int listPosition = ((Integer) CG.getTermNumbers().get(node))
                    .intValue();
            ...
            for (int i = 0; i < temp.length; i++) {
                ...
                if (!type_name.equals("null")) {
                    if (type_name.startsWith("L")) {
                        macros.apply(file, "ParserTypedLinkedListAddAll",
                                new String[] { "list", "" + listPosition,
                                        "list", "" + position });
                    } else {
                        macros.apply(
                                file,
    ...
    public void outANewTerm(ANewTerm node) {
        ...
        try {
            ...
            macros.apply(file, "ParserNewBodyNew", new String[] { type_name,
                    "" + position, newAltName });
            ...
            if (node.getParams().size() > 0) {
                ...
                for (int i = 0; i < temp.length; i++) {
                    if (simpleTermTransformMap.get(temp[i]) != null) {
                        type_name = (String) simpleTermTransformMap
                                .get(temp[i]);
                    } else {
                        type_name = (String) CG.getAltTransformElemTypes().get(
                                temp[i]);
                    ...
                    if (type_name.equals("null")) {
                        macros.apply(file, "ParserNew&ListBodyParamsNull",
                                new String[] { isNotTheFirstParam + "null" });
                    } else {
                        if (type_name.startsWith("L")) {
                            type_name = "list";
                        } else {
                        ...
                        macros.apply(file, "ParserNew&ListBodyParams",
                                new String[] { isNotTheFirstParam + type_name,
                                        "" + position });
                    }
    ...
    public String name(AProdName node) {
        if (node.getProdNameTail() != null) {
            return "A" + ResolveIds.name(node.getProdNameTail().getText())
